...
assert.match(method01Logic, /what-causal-flow/, 'method 01 logic must use the WHAT causal flow');
assert.match(method01Logic, /causal-arrow-stack/, 'method 01 logic must include the middle model-building arrow');
assert.match(method01Logic, /METHOD 1<br\/>MODEL BUILDING/, 'method 01 arrow must label the model-building step');
...
assert.match(method02Logic, /twist-arrow-stack/, 'method 02 logic must include the middle hypothesis-formulation arrow');
assert.match(method02Logic, /METHOD 2<br\/>HYPOTHESIS FORMULATION/, 'method 02 arrow must label hypothesis formulation');
assert.match(method02Logic, /hypothesis-block-list/, 'method 02 right-side hypothesis cards must remain visible');
assert.match(method02Logic, /Minimize text volume|Sequence sign placements|Integrate visual graphics|Place visual anchors/, 'method 02 right-side cards must include explanatory descriptions');
...
assert.match(html, /Memory-oriented signage system design principle 1/i, 'method 03 output must reduce hypotheses into validated principles');
...
assert.match(method03Logic, /method03-flow/, 'method 03 logic must use the validation flow');
assert.doesNotMatch(method03Logic, /block-funnel-horizontal/, 'method 03 logic must not use the removed funnel widget');
assert.match(method03Logic, /block-arrow-stack method03-filter/, 'method 03 logic must include the middle field-validation arrow');
assert.match(method03Logic, /class="method03-funnel-svg"/, 'method 03 logic must include the funnel SVG arrow widget');
